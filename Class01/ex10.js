...
addReminderBtn.addEventListener('click', function addReminder() {
    ...
    showAllBtn.addEventListener('click', function () {
        ...
        for (let i = 0; i < arrayOfReminders.length; i++) {
            let titleColor = arrayOfReminders[i].color

            element += `<tr>`
            // tableDiv.innerText.style.color = `${arrayOfReminders[i].color}`           // how to change color ?!?!
            element += `<td>${arrayOfReminders[i].title}</td>`
            // tableDiv.innerText.style.color = `black`
